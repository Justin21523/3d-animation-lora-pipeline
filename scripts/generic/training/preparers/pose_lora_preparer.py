"""
Pose LoRA data preparer.

Prepares training data for pose-specific LoRA by:
1. Detecting poses/keypoints from character images
2. Clustering by pose similarity
3. Filtering low-quality poses
4. Generating pose-aware captions
...
class PoseLoRAPreparer:
    """
    Pose LoRA data preparation pipeline.

    This preparer organizes images by pose/keypoint similarity:
    - Pose detection (MediaPipe/RTMPose/OpenPose)
    - Pose feature extraction (keypoint normalization)
    - Pose clustering (by angle, view, action)
    - Pose-aware caption generation
    ...
    def _initialize_components(self):
        ...
        # Feature extractor (CLIP for visual similarity)
        ...
        # Clusterer (pose-based clustering)
        ...
        # Caption engine (pose-aware templates)
    ...
    def prepare(self) -> Dict[str, Any]:
        """
        Execute the full pose LoRA data preparation pipeline.

        Returns:
            Dictionary with preparation results and metadata
        """
        self.logger.info("=" * 60)
        self.logger.info(f"Pose LoRA Preparation: {self.character_name}")
        self.logger.info("=" * 60)
        ...
        # Step 3: Feature extraction (visual features for pose grouping)
        features = self._extract_features(image_paths)
        self.logger.info(f"Step 3: Extracted features with shape {features.shape}")

        # Step 4: Clustering by pose similarity
        labels = self._cluster_features(features)
        self.logger.info(f"Step 4: Clustered into {self.clusterer.n_clusters_} pose groups")

        # Step 5: Organize by pose cluster
        pose_groups = self._organize_clusters(image_paths, labels)
        self.logger.info(f"Step 5: Organized {len(pose_groups)} pose groups")

        # Step 6: Generate pose-aware captions
        self._generate_captions(pose_groups)
        self.logger.info(f"Step 6: Generated captions for all pose groups")

        # Step 7: Assemble dataset
        dataset_info = self._assemble_dataset(pose_groups)
    ...
    def _cluster_features(self, features: np.ndarray) -> np.ndarray:
        """Cluster features by pose similarity."""
        self.logger.info("Clustering by pose...")
    ...
    def _organize_clusters(
        ...
        """Organize images by pose cluster label."""
        ...
        for path, label in zip(image_paths, labels):
            ...
            pose_groups[label].append(path)

        # Sort by size (largest first)
        pose_groups = dict(sorted(pose_groups.items(), key=lambda x: len(x[1]), reverse=True))

        return pose_groups

    def _generate_captions(self, pose_groups: Dict[int, List[Path]]):
        """Generate pose-aware captions for all images."""
        self.logger.info("Generating pose-aware captions...")

        # Process all images
        all_images = []
        for group_id, images in pose_groups.items():
    ...
    def _assemble_dataset(self, pose_groups: Dict[int, List[Path]]) -> Dict[str, Any]:
        """Assemble final dataset in Kohya format."""
        self.logger.info("Assembling pose dataset...")

        # Kohya format: output_dir / {repeats}_{character_name}_poses / images and captions
        repeats = self.config.get('repeats', 10)
        dataset_dir = self.output_dir / f"{repeats}_{self.character_name}_poses"
        dataset_dir.mkdir(parents=True, exist_ok=True)

        # Copy all images and captions
        total_images = 0

        for group_id, image_paths in pose_groups.items():
        ...
        dataset_info = {
            ...
            'num_pose_groups': len(pose_groups),
            'repeats': repeats,
            'group_sizes': {int(k): len(v) for k, v in pose_groups.items()}
        }
...
# CLI entry point
def main():
    """Command-line interface for pose LoRA preparer."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Prepare pose-specific LoRA training data"
    )
