"""
Expression LoRA data preparer.

Prepares training data for expression-specific LoRA by:
1. Extracting facial features from character images
2. Clustering by expression similarity (happy, sad, angry, etc.)
3. Filtering low-quality/occluded faces
4. Generating expression-aware captions
...
from quality_filters import BlurFilter, SizeFilter


class ExpressionLoRAPreparer:
    """
    Expression LoRA data preparation pipeline.

    This preparer organizes images by facial expression:
    - Face detection and cropping
    - Expression feature extraction (CLIP/facial embeddings)
    - Expression clustering (happy, neutral, sad, angry, surprised, etc.)
    - Expression-aware caption generation
    ...
    def __init__(
        ...
        """
        Initialize expression LoRA preparer.
    ...
    def _initialize_components(self):
        ...
        # Feature extractor (CLIP for facial expression features)
        ...
        # Clusterer (expression-based clustering)
        ...
        if not filter_configs:
            # Default filters for expression LoRA (strict for face quality)
            filter_configs = [
        ...
        # Caption engine (expression-aware templates)
        ...
        # Add expression-specific prefix
        if 'prefix' not in caption_config:
            caption_config['prefix'] = 'a 3d animated character'
    ...
    def prepare(self) -> Dict[str, Any]:
        """
        Execute the full expression LoRA data preparation pipeline.

        Returns:
            Dictionary with preparation results and metadata
        """
        self.logger.info("=" * 60)
        self.logger.info(f"Expression LoRA Preparation: {self.character_name}")
        self.logger.info("=" * 60)
        ...
        # Step 3: Feature extraction (facial expression features)
        features = self._extract_features(image_paths)
        self.logger.info(f"Step 3: Extracted features with shape {features.shape}")

        # Step 4: Clustering by expression similarity
        labels = self._cluster_features(features)
        self.logger.info(f"Step 4: Clustered into {self.clusterer.n_clusters_} expression groups")

        # Step 5: Organize by expression cluster
        expression_groups = self._organize_clusters(image_paths, labels)
        self.logger.info(f"Step 5: Organized {len(expression_groups)} expression groups")

        # Step 6: Generate expression-aware captions
        self._generate_captions(expression_groups)
        self.logger.info(f"Step 6: Generated captions for all expression groups")

        # Step 7: Assemble dataset
        dataset_info = self._assemble_dataset(expression_groups)
    ...
    def _extract_features(self, image_paths: List[Path]) -> np.ndarray:
        ...
        self.logger.info("Extracting facial expression features...")
    ...
    def _cluster_features(self, features: np.ndarray) -> np.ndarray:
        """Cluster features by expression similarity."""
        self.logger.info("Clustering by expression...")
    ...
    def _organize_clusters(
        ...
        """Organize images by expression cluster label."""
        ...
        for path, label in zip(image_paths, labels):
            ...
            if label not in expression_groups:
                expression_groups[label] = []

            expression_groups[label].append(path)

        # Sort by size (largest first)
        expression_groups = dict(sorted(expression_groups.items(), key=lambda x: len(x[1]), reverse=True))

        return expression_groups

    def _generate_captions(self, expression_groups: Dict[int, List[Path]]):
        """Generate expression-aware captions for all images."""
        self.logger.info("Generating expression-aware captions...")

        # Process all images
        all_images = []
        for group_id, images in expression_groups.items():
    ...
    def _assemble_dataset(self, expression_groups: Dict[int, List[Path]]) -> Dict[str, Any]:
        """Assemble final dataset in Kohya format."""
        self.logger.info("Assembling expression dataset...")

        # Kohya format: output_dir / {repeats}_{character_name}_expressions / images and captions
        repeats = self.config.get('repeats', 10)
        dataset_dir = self.output_dir / f"{repeats}_{self.character_name}_expressions"
        dataset_dir.mkdir(parents=True, exist_ok=True)

        # Copy all images and captions
        total_images = 0

        for group_id, image_paths in expression_groups.items():
        ...
        dataset_info = {
            ...
            'num_expression_groups': len(expression_groups),
            'repeats': repeats,
            'group_sizes': {int(k): len(v) for k, v in expression_groups.items()}
        }
...
# CLI entry point
def main():
    """Command-line interface for expression LoRA preparer."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Prepare expression-specific LoRA training data"
    )
    ...
    print("\n" + "=" * 60)
    print("✓ Expression LoRA preparation completed!")
    print("=" * 60)
    print(f"Dataset: {metadata['dataset_info']['dataset_dir']}")
    print(f"Images: {metadata['dataset_info']['num_images']}")
    print(f"Expression groups: {metadata['dataset_info']['num_expression_groups']}")
    print("=" * 60)
