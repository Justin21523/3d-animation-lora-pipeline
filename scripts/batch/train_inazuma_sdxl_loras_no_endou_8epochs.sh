#!/bin/bash
#
# Inazuma Eleven SDXL LoRA Batch Training Script (NO ENDOU)
# Sequential training for all Inazuma Eleven characters EXCEPT Endou Mamoru.
#
# This run is intended for the current Inazuma SDXL configs which:
#   - Use 8 epochs
#   - Use caption files with `caption_extension = ".txt"`
#
# Usage:
#   tmux new-session -s inazuma_training_no_endou
#   bash scripts/batch/train_inazuma_sdxl_loras_no_endou_8epochs.sh
#
...
# Character list (training order) - excludes endou_mamoru
CHARACTERS=(
    "gouenji_shuuya"
    "fudou_akio"
    "matsukaze_tenma"
...
# Batch log file
BATCH_LOG="$LOG_DIR/inazuma_batch_training_no_endou_8epochs_$(date +%Y%m%d_%H%M%S).log"
...
train_character() {
    ...
    local char_log="$LOG_DIR/inazuma_${char_id}_sdxl_training_no_endou_8epochs_$(date +%Y%m%d_%H%M%S).log"
...
log "=========================================="
log "Inazuma Eleven SDXL LoRA Batch Training (NO ENDOU, 8 EPOCHS)"
log "=========================================="
...
log "2. Evaluate checkpoints (optional):"
log "   python scripts/batch/evaluate_inazuma_loras.py"
log "=========================================="
